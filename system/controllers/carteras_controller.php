	...
	*/
	class Carteras_Controller{
        ...
        */
         ...
         * 
        */
        public function select_all_by_id_cartera_not_in_join_usuario_cartera($id_usuario){
			$conexion = new Conexion();
			$cn = $conexion->connect();

			$query = $cn->prepare("SELECT * FROM " . $this->table_name . " WHERE id_cartera NOT IN (SELECT id_cartera FROM usuario_cartera WHERE id_usuario = ?)");
